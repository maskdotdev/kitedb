/**
 * Ray Database Benchmark - Single File Format
 *
 * Identical to benchmark.ts but uses the single-file .raydb format.
 * Run both benchmarks to compare multi-file vs single-file performance.
 *
 * Usage:
 *   bun run bench/benchmark-single-file.ts [options]
 *
 * Options:
 ...
 *   --output FILE     Output file path (default: bench/results/benchmark-single-file-<timestamp>.txt)
...
function parseArgs(): BenchConfig {
  ...
  const defaultOutput = join(
    ...
    "results",
    `benchmark-single-file-${timestamp}.txt`,
  );
...
async function buildRealisticGraph(
  ...
  // Use large batch sizes for setup (auto-checkpoint handles WAL overflow)
...
async function benchmarkDeltaImpact(config: BenchConfig): Promise<void> {
  ...
  for (const deltaPercent of deltaPercents) {
    // Use single-file format: file path instead of directory
    const testPath = join(tmpdir(), `ray-delta-sf-${deltaPercent}-${Date.now()}.raydb`);

    try {
      const db = await openGraphDB(testPath, {
        autoCheckpoint: true,
        checkpointThreshold: 0.8,
        // Default 1MB WAL - smaller batches keep us within secondary region
      });
...
// =============================================================================
// Database Size Report (Single File)
// =============================================================================
...
async function reportDatabaseSize(
  ...
  logger.log(`  Format:          Single file (.raydb)`);
...
// =============================================================================
// Write Performance Benchmark with Checkpoint Tracking
// =============================================================================

interface CheckpointMetrics {
  checkpointCount: number;
  totalCheckpointDurationMs: number;
  writesWhileCheckpointing: number;
}
...
async function benchmarkWrites(
  ...
  logger.log("\n--- Write Performance (Buffered, no auto-checkpoint) ---");

  // Batch transactions
  // Note: Total writes limited to avoid WAL overflow (default 1MB WAL ≈ ~768KB primary region)
  // We write across 3 batch sizes (10, 100, 1000), so limit each to ~2000 nodes
  // Auto-checkpoint is NOT enabled here - see Background Checkpoint benchmark for that
  const batchSizes = [10, 100, 1000];
  for (const batchSize of batchSizes) {
    const tracker = new LatencyTracker();
    
    // Limit total nodes per batch size to ~2000 to stay within WAL capacity across all sizes
    const maxNodesPerBatchSize = 2000;
    const batches = Math.min(Math.floor(iterations / batchSize), Math.floor(maxNodesPerBatchSize / batchSize));
...
// =============================================================================
// Background Checkpoint Performance Benchmark
// =============================================================================

async function benchmarkBackgroundCheckpoint(config: BenchConfig): Promise<void> {
  logger.log("\n--- Background Checkpoint Performance ---");
  logger.log("  Testing write performance with auto-checkpoint enabled...");

  // Use larger WAL to allow background checkpoint to complete before secondary fills
  const testPath = join(tmpdir(), `ray-bgcp-${Date.now()}.raydb`);
  const walSizeKB = 2048; // 2MB WAL for reliable background checkpoint behavior

  try {
    const db = await openGraphDB(testPath, {
      autoCheckpoint: true,
      checkpointThreshold: 0.8, // Trigger at 80% WAL usage
      walSize: walSizeKB * 1024,
    });

    // Create edge types
    const tx0 = beginTx(db);
    const etype = defineEtype(tx0, "RELATES");
    await commit(tx0);

    // Benchmark batch writes with auto-checkpoint
    ...
    for (const batchSize of batchSizes) {
      ...
      const checkpointMetrics: CheckpointMetrics = {
        checkpointCount: 0,
        totalCheckpointDurationMs: 0,
        writesWhileCheckpointing: 0,
      };
      
      const batches = Math.min(Math.floor(config.iterations / batchSize), 50); // Fewer batches to avoid WAL overflow
      let activeCheckpointStart: number | null = null;
      let nodeCounter = 0;
      
      for (let b = 0; b < batches; b++) {
        // Check if a checkpoint started/completed
        const wasCheckpointing = activeCheckpointStart !== null;
        const checkpointRunning = isCheckpointRunning(db);
        
        if (checkpointRunning && !wasCheckpointing) {
          activeCheckpointStart = performance.now();
          checkpointMetrics.checkpointCount++;
        } else if (!checkpointRunning && wasCheckpointing) {
          checkpointMetrics.totalCheckpointDurationMs += performance.now() - activeCheckpointStart!;
          activeCheckpointStart = null;
        }
        
        if (checkpointRunning) {
          checkpointMetrics.writesWhileCheckpointing++;
        }
        
        const start = Bun.nanoseconds();
        const tx = beginTx(db);
        for (let i = 0; i < batchSize; i++) {
          createNode(tx, { key: `bgcp:${batchSize}:${nodeCounter++}` });
        }
        await commit(tx);
        tracker.record(Bun.nanoseconds() - start);
      }
      ...
      const st = tracker.getStats();
      const opsPerSec = st.sum > 0 ? (batchSize * st.count) / (st.sum / 1_000_000_000) : 0;
      const label = `Batch of ${batchSize.toString().padStart(4)} nodes (auto-checkpoint)`.padEnd(55);
      logger.log(
        `${label} p50=${formatLatency(st.p50).padStart(10)} p95=${formatLatency(st.p95).padStart(10)} (${formatNumber(Math.round(opsPerSec))} nodes/sec)`,
      );
      
      if (checkpointMetrics.checkpointCount > 0) {
        const avgCheckpointMs = checkpointMetrics.totalCheckpointDurationMs / checkpointMetrics.checkpointCount;
        logger.log(
          `    Checkpoints: ${checkpointMetrics.checkpointCount} @ avg ${avgCheckpointMs.toFixed(1)}ms (${checkpointMetrics.writesWhileCheckpointing} writes during checkpoint)`,
        );
      }
    }
...
async function runBenchmarks(config: BenchConfig) {
  ...
  logger.log("Ray Database Benchmark - Single File Format (.raydb)");
  logger.log("=".repeat(120));
  ...
  // Use single-file format: file path (not directory) with .raydb extension
  const testPath = join(tmpdir(), `ray-bench-sf-${Date.now()}.raydb`);
  ...
  try {
    ...
    const db = await openGraphDB(testPath, {
      autoCheckpoint: true,
      checkpointThreshold: 0.8,
      // Default 1MB WAL - smaller batches keep us within secondary region capacity
    });
    ...
    await reportDatabaseSize(testPath, config);

    // Delta impact (separate instances)
    logger.log("\n[Bonus] Delta impact...");
    await benchmarkDeltaImpact(config);
    
    // Background checkpoint benchmark
    logger.log("\n[Bonus] Background checkpoint performance...");
    await benchmarkBackgroundCheckpoint(config);
  } finally {
    if (config.keepDb) {
